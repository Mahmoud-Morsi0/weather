...
btn.addEventListener("click", async function () {
  var res = await fetch(
    `http://api.weatherapi.com/v1/forecast.json?key=4ded15e9891d46979b5213129240901&q=${search.value}&days=7`
  );
  ...
  div[0].innerHTML = `
    ...
    <img src="${finalRes.forecast.forecastday[0].day.condition.icon}" alt="icon">  
  ...
  for (let i = 1; i < 8; i++) {
    ...
    <img src="${finalRes.forecast.forecastday[i].day.condition.icon}" alt="icon">                        
    ${finalRes.location.name}<br> 
...
(async function () {
  var res = await fetch(`http://api.weatherapi.com/v1/forecast.json?key=4ded15e9891d46979b5213129240901&q=cairo&days=7`);
  finalRes = await res.json();
  console.log(finalRes);
  var d = new Date(`${finalRes.forecast.forecastday[0].date}`);
  var dayName = days[d.getDay()];
